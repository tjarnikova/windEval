...
def get_gaussian_kde(prod = 'ERA5', seas = 'FY', yst = 1940, yen = 1949, xmi = 0, xma = 25):
    ...
    savenam = f'KDE-{prod}-{seas}-{yst}-{yen}-0-25.nc'#seas
    print(savenam)
    ...
    kde_x = np.linspace(xmi, xma, 125)
    kde = gaussian_kde(np.ravel(tval), weights=np.ravel(timask))
    kde_val = kde(kde_x)

    binsback, bin_cent, histback, = custhist(np.ravel(tval), 125, xmi, xma, tweights = np.ravel(timask))
